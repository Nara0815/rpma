...
void *thr_func(void *arg)
{
 ...
 for(k = start; k < end; k++){
		...
		ret = rpma_read(conn, dst_mr, tid*2*KILOBYTE, src_mr, idx*64, 2*KILOBYTE,
	...
	if (wc.opcode != IBV_WC_RDMA_READ) {
		...
		for(int i=0; i<metadata_size; i++){
			...
			metadata[i] = ((char *)dst_ptr)[tid*2*KILOBYTE+i];
			//printf( "Read message meta: %c\n", metadata[i]);
      ...
      */
            ...
            if(hopinfo == 1){
				...
				for(int i=0; i<keylen; i++){
					...
					key[i] = ((char *)dst_ptr)[tid*2*KILOBYTE+metasize+hopsize+i];
					//printf( "Read message meta: %c\n", key[i]);
                ...
                for ( i = 0; i < HOP_NUMBER; i++ ) {
                    if ( hopinfo & (1 << i) ) {
						...
						for(int j=0; j<keylen; j++){
							...
							key[j] = ((char *)dst_ptr)[tid*2*KILOBYTE+metasize+hopsize+i*64+j];
...
for(int i = 0; i< KEY_NUMBERS; i++){
	...
	/* allocate a memory */
	dst_ptr = malloc_aligned(NUM_THREADS*2*KILOBYTE);
	if (dst_ptr == NULL) {
		ret = -1;
	...
	/* register the memory */
	ret = rpma_mr_reg(peer, dst_ptr, NUM_THREADS*2*KILOBYTE, RPMA_MR_USAGE_READ_DST,
				&dst_mr);
	if (ret)
		goto err_mr_free;
